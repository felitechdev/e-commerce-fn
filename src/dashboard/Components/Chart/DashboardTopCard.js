    ...
    <Row gutter={[16, 16]} className="w-full">
      ...
      <Col xs={24} sm={12} md={6}>
        ...
        >
          ...
          <div className="dashboard-top-card__value ">
            Amount: {totalMonthlyEarning} RWF
          </div>
          <div className="dashboard-top-card__subtitle">
      ...
      <Col xs={24} sm={12} md={6}>
        ...
        >
          ...
          <div
            className="dashboard-top-card__value "
            style={{ background: monthlyEarningBgColor }}
          >
            {totalYearlyOrders}
          </div>
...
export const OrdersLineCahrt = () => {
  ...
  const now = new Date();
  const oneDayAgo = new Date(now.setDate(now.getDate()));

  const totalDailyOrders = orders.filter(
    (order) => new Date(order.createdAt).getDate() == oneDayAgo.getDate()
  ).length;
  const totalDailyEarning = orders
    .filter(
      (order) => new Date(order.createdAt).getDate() == oneDayAgo.getDate()
...
export const SellerPieChart = () => {
  ...
  return (
    <div className=" shadow-lg border w-[100%] sml:w-[40%]  mt-5    overflow-auto rounded-lg px-3   bg-[#e1e7f1]">
