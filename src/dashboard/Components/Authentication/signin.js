...
const SignInForm = (props) => {
  ...
  if (existingToken) {
    ...
    console.log("token removed", existingToken);
  }
      ...
      Dispatch(Login(userData))
        ...
        .then((response) => {
          ...
          if (response.status == 200) {
            ...
            Cookies.set("profile", { user: response?.data?.data?.user });
        ...
        .catch((error) => {
          ...
          if (error.status == 401) {
            setErrorAlert({
              status: true,
              message: `${("you are not allowed", error.message)}`,
            });
          }
          setAlertIndex("error"); // Display error alert on error
          setAlertDescription("Error: " + error.message);
          setErrorAlert({ status: true, message: "you are not allowed" });
  ...
  useEffect(() => {
    if (
      ...
      setErrorAlert({ status: true, message: "welcome to admin dashboard" });
      navigate("/dashboard");
        ...
        <div className="px-6 w-full h-[90%] flex flex-col justify-center ">
          ...
          <div className="flex flex-col gap-3 ">
            ...
            <div className="flex flex-col gap-.5">
              ...
              />
              {errEmail && (
                <p className="text-xl text-red-500 text-[red] font-titleFont font-semibold px-4">
            ...
            <div className="flex flex-col gap-.5">
              ...
              {errPassword && (
                <p className="text-xl text-red-500 text-[red] font-titleFont font-semibold px-4">
                  <span className="font-bold italic  text-[red]  mr-1">!</span>
                  {errPassword}
                </p>
              )}
            </div>
            ...
            <p className="text-xl text-center font-titleFont font-medium -mt-2">
              ...
              <span
                ...
                onClick={() =>
                  props.setOpenForm({ signin: false, signup: true })
                }
              >
                Sign up
              </span>
