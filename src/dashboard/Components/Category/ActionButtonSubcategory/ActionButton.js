...
export const ActionButtonSubCategory = (props) => {
          ...
          dispatch(deletesubcategory({ id: categoryId, token: token }))
            ...
            .then((response) => {
              if (response.status === 204) {
                setOnSuccess("Sub Category deleted successfully");
  ...
  return (
    <>
      ...
      >
        <SubCategory openUPdate={true} categoryId={props.categoryId} />
      </Modal>
      <div div className="flex justify-start  space-x-5 text-lg  ">
        <EditFilled
          className=" text-icon1 ml-3 mr-2"
          onClick={() => handleOpen()}
        />
        <DeleteFilled
