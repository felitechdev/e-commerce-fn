...
const UpdateProductModel = (props) => {
  ...
  useEffect(() => {
    ...
    setOtherImageUrls(
      DBProductInfo?.productImages?.otherImages.map(
        (image) => image.url
      )
    );
    ...
    setValue(
      'discountPercentage',
      DBProductInfo?.discountPercentage || 0
    );
    setValue(
      'otherImages',
      DBProductInfo.productImages.otherImages || ''
    );
    setValue('brandName', DBProductInfo.brandName || '');
    setValue(
      'stockQuantity',
      DBProductInfo.stockQuantity || ''
    );
    setValue(
      'quantityParameter',
      DBProductInfo.quantityParameter || ''
    );
    setValue('hasColors', DBProductInfo.hasColors || '');
    setValue(
      'hasMeasurements',
      DBProductInfo.hasMeasurements || ''
    );
