...
const CompanyModel = (props) => {
  ...
  const onFinish = async (values) => {
    ...
    if (values.accountHolderName) {
      payload.bankAccount.accountHolderName =
    ...
    dispatch(
      Updateprofile({ data: formData, token: token })
    )
      .unwrap()
      .then((res) => {
  ...
  useEffect(() => {
    ...
    setValue(
      'companyName',
      props.profileview?.data?.profile.companyName || ''
    );
    setValue(
      'phoneNumber',
      props.profileview?.data?.profile.phoneNumber || ''
    );
    setValue(
      'companyEmail',
    ...
    setValue(
      'website',
      props.profileview?.data?.profile.website || ''
    );
    ...
    setValue(
      'logo',
      props.profileview?.data?.profile.logo || ''
    );
    setValue('accountNumber', accountNumber);
    setValue(
      'cardNumber',
      props.profileview?.data?.profile.cardNumber || ''
    ); // Assuming cardNumber is a string
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h1 className=' font-bold'>Business Info</h1>{' '}
          <hr className='h-2 ' />
          <Row
            gutter={[16, 16]}
